...
read_excel("data/Weights by Commodity Group for CPI_2012 and 2006 based, Philippines, NCR, and AONCR_2.xlsx", 
  ...
  mutate(genus = if_else(grepl("^.*\\. .*$", item), item, NA_character_)) %>% 
  fill(genus) %>% 
  mutate(n = 1:n(), 
         family = if_else(grepl("^\\* .*", item), item, NA_character_)) %>% 
  fill(family) %>% 
  mutate(family = if_else(n < 16, family, NA_character_)) %>% 
  ...
  select(-item, -n) %>% 
  select(genus, family, species, w_2006, w_2012) -> weights
# Add one more category for Bread and cereals
```
